...
async function getRemolquesInfo(id_CartaPorte) {
  try {
    let query = "SELECT cat_tiporemolque.st_ClaveRemolque, inner_query.st_placa FROM cat_tiporemolque  INNER JOIN ( SELECT tbl_remolques.id_Remolque, tbl_remolques.id_TipoRemolque, tbl_remolques.st_placa FROM tbl_remolques   RIGHT JOIN(    SELECT rel_viaje_remolque.id_Remolque FROM tbl_viaje  LEFT JOIN tbl_cartaporte ON tbl_cartaporte.id_Viaje=tbl_viaje.id_Viaje LEFT JOIN rel_viaje_remolque ON rel_viaje_remolque.id_Viaje = tbl_viaje.id_Viaje    WHERE tbl_cartaporte.id_CartaPorte= :id  ) AS subquery ON tbl_remolques.id_Remolque = subquery.id_Remolque ) as inner_query ON inner_query.id_TipoRemolque  = cat_tiporemolque.id_TipoRemolque;";
 ...
 if (Object.keys(TotalImpuestosAttribute).length > 0) {
    ...
    if(RemolqueRequirementValidation != 0 ){
      ...
      if(remolquesArray.length > 0){
        ...
        console.log("No hay remolques asignados a la unidad.")
      }
      }
